...
<div class="flex flex-col menu" x-data="Menu" :class="{ 'is-top-hidden': topHidden }">
    <div class="flex-1 py-3 nav__top bg-primary">
        <div class="flex flex-col items-center justify-between site-branding section-container--sm sm:flex-row">
            <div class="logo">
    ...
    get_template_directory() . "/resources/static/svg/ch-logo.svg",
            ...
            <div class="flex items-center search-bar-and-address">
                <?php if (
                    TAW\Core\OptionsPage\OptionsPage::get("company_address")
                ): ?>
                    <div class="text-white! pr-4 mr-4 border-r text-xs text-right leading-5" href="tel:<?php echo esc_attr(
                        TAW\Core\OptionsPage\OptionsPage::get(
                            "company_address",
                        ),
                    ); ?>">
                ...
                <?php endif; ?>
                <button class="text-white cursor-pointer search-icon" @click="open = true" aria-label="Abrir búsqueda">
                    ...
                    <?php foreach ($suggested_pages as $page): ?>
                        <li>
                            <a href="<?php echo esc_url(
                                ...
                                <span class="search-overlay__result-title"><?php echo esc_html(
                                    $page["title"],
                                ); ?></span>
                            </a>
                        </li>
    ...
    <div class="flex-1 py-2 nav__bottom sm:py-4 bg-secondary">
        <div class="flex flex-wrap items-center justify-center gap-4 section-container--sm sm:justify-between">
            <?php
                ...
                <?php if ($menu && $menu->hasItems()): ?>
                    <nav class="flex-wrap items-center hidden sm:flex gap-x-8 gap-y-3">
                        <?php foreach ($menu->items() as $item):
                            ...
                            <div class="relative group" <?php if (
                                $item->hasChildren()
                            ): ?>x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false" <?php endif; ?>>
                                <a href="<?php echo esc_url(
                                    $item->url(),
                                ); ?>" class="menu__item flex items-center gap-1.5 text-white transition-colors text-xs uppercase <?php echo $item->isInActiveTrail()
    ...
    : "opacity-85 hover:opacity-100"; ?>">
                                    ...
                                    <?php if ($item->hasChildren()): ?>
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="mt-px transition-transform duration-200 size-3 group-hover:rotate-180">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                                        </svg>
                                    <?php endif; ?>
                                </a>
                                <?php if ($item->hasChildren()): ?>
                                    <div x-show="open" x-transition class="absolute left-0 z-50 pt-6 submenu top-full min-w-68">
                                        <div class="p-3 bg-white border border-gray-200 rounded-md shadow-xl">
                                            <?php foreach (
                                                $item->children()
                                                as $child
                                            ): ?>
                                                <?php $child_icon = get_post_meta(
                                                    $child->wpPost()->ID,
                                                    "_taw_menu_item_icon",
                                                    true,
                                                ); ?>
                                                <a href="<?php echo esc_url(
                                                    ...
                                                    class="menu__item flex items-center gap-2 p-2 border-sm text-sm text-gray-700 hover:bg-gray-100 <?php echo $child->isInActiveTrail()
                                                        ? "bg-gray-100"
                                                        : ""; ?>">
                                                    <?php if ($child_icon): ?>
            ...
            <div class="items-center justify-center hidden contact sm:flex">
                <?php if (
                    TAW\Core\OptionsPage\OptionsPage::get("general_contact_email")
                ): ?>
                    <a class="flex items-center justify-center gap-1 pr-4 mr-4 text-xs text-white border-r border-white" href="mailto:<?php echo esc_attr(
                        TAW\Core\OptionsPage\OptionsPage::get("general_contact_email"),
                    ); ?>">
                ...
                <?php endif; ?>
                <?php if (
                    TAW\Core\OptionsPage\OptionsPage::get("general_contact_phone")
                ): ?>
                    <a class="flex items-center justify-center gap-1 text-xs text-white" href="tel:<?php echo esc_attr(
                        TAW\Core\OptionsPage\OptionsPage::get("general_contact_phone"),
                    ); ?>">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-5">
    ...
    <div
        ...
        <div class="mobile-drawer-panel__header">
            <a href="<?php echo esc_url(
                home_url("/"),
            ); ?>" class="mobile-drawer-panel__logo">
                <?php echo file_get_contents(
                    get_template_directory() .
                        "/resources/static/svg/ch-logo.svg",
                ); ?>
            </a>
            <button
                        ...
                        <?php if ($item->hasChildren()): ?>
                            <div class="mobile-drawer-panel__row">
                                <a href="<?php echo esc_url($item->url()); ?>"
                                    class="mobile-drawer-panel__link flex items-center gap-2 <?php echo $item->isInActiveTrail()
                                        ? "is-active"
                                        : ""; ?>">
                                    <?php if (
                                        $mobile_icon
                                ...
                                <button
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="transition-transform duration-200 size-4" :class="{ 'rotate-180': subOpen }">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                                    </svg>
                                </button>
                            </div>
                            <ul
                                ...
                                <?php foreach ($item->children() as $child): ?>
                                    <?php $child_mobile_icon = get_post_meta(
                                        $child->wpPost()->ID,
                                        "_taw_menu_item_icon",
                                        true,
                                    ); ?>
                                    <li>
                                        <a href="<?php echo esc_url(
                                            ...
                                            class="mobile-drawer-panel__sublink flex items-center gap-2 <?php echo $child->isActive()
                                                ? "is-active"
                                                : ""; ?>">
        ...
        <div class="mobile-drawer-panel__contact">
            <?php if (
                TAW\Core\OptionsPage\OptionsPage::get("company_email")
            ): ?>
                <a class="mobile-drawer-panel__contact-link" href="mailto:<?php echo esc_attr(
                    TAW\Core\OptionsPage\OptionsPage::get("company_email"),
                ); ?>">
            ...
            <?php endif; ?>
            <?php if (
                TAW\Core\OptionsPage\OptionsPage::get("company_phone")
            ): ?>
                <a class="mobile-drawer-panel__contact-link" href="tel:<?php echo esc_attr(
                    TAW\Core\OptionsPage\OptionsPage::get("company_phone"),
                ); ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 shrink-0">
