...
/**
 * HighlightsRow Block Template
 *
 * Horizontal row of icon + title + description cards.
 * Icons are inline SVGs mapped from the predefined admin select options.
 *
 * @var string $heading
 * @var string $subheading
 * @var array  $items  Each: icon (string key), title (string), description (string)
 */
...
$icons = [
    ...
    'phone' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.18h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.73a16 16 0 0 0 6.29 6.29l.86-.86a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7a2 2 0 0 1 1.72 2.01z"/></svg>',
...
?>
<section class="highlights-row ch-section colored bg-primary">
    <div class="section-container--sm">

        <?php if ($heading || $subheading) : ?>
        <div class="highlights-row__header">
            <?php if ($heading) : ?>
                <h2 class="highlights-row__heading"><?php echo esc_html($heading); ?></h2>
            <?php endif; ?>
            <?php if ($subheading) : ?>
                <p class="highlights-row__subheading"><?php echo esc_html($subheading); ?></p>
            <?php endif; ?>
        </div>
        <?php endif; ?>

        <div class="highlights-row__grid">
            <?php foreach ($items as $item) :
                $svg = $icons[$item['icon']] ?? $icons['check'];
            ?>
            <div class="highlight-card">
                <div class="highlight-card__icon" aria-hidden="true">
                    <?php echo $svg; // phpcs:ignore — safe inline SVG from class constant ?>
                </div>
                <h3 class="highlight-card__title"><?php echo esc_html($item['title']); ?></h3>
                <?php if ($item['description']) : ?>
                    <p class="highlight-card__description"><?php echo esc_html($item['description']); ?></p>
                <?php endif; ?>
            </div>
            <?php endforeach; ?>
        </div>
